...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Tapez votre question...' +'"'+ '>' +
...
addChatbotMessage("Bonjour! Je suis le chatbot. Comment puis-je t'aider? (le premier message peut prendre quelques secondes)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_be.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Récupération de la réponse...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Question:</strong> ' + question + '<br><strong>Répondre:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Impossible de récupérer la réponse.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
